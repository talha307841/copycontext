...
(function() {
  ...
  function createButton() {
    try {
      ...
      // Fallback mode: keep extension usable even if page scripts interfere with styled UI creation.
      const fallback = document.createElement('button');
      fallback.id = 'contextshift-floating-btn';
      fallback.textContent = 'Capture';
      fallback.style.cssText = 'position:fixed;right:16px;bottom:16px;z-index:99999;padding:10px 14px;border-radius:12px;border:1px solid #7c5cfc;background:#7c5cfc;color:#fff;cursor:pointer;';
      fallback.onclick = () => {
        chrome.runtime.sendMessage({ action: 'CAPTURE_AND_STORE_FROM_TAB' }, () => {});
      };
      if (!document.getElementById('contextshift-floating-btn')) {
        (document.body || document.documentElement).appendChild(fallback);
      }
      console.warn('ContextShift overlay fallback mode:', err?.message || err);
    }
  }
